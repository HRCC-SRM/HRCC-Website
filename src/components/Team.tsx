...
const TeamMember: React.FC<TeamMemberProps> = ({ name, role, image, socials }) => (
  <div className="group relative overflow-hidden rounded-xl bg-gray-900 transition-all duration-300 hover:scale-105">
    <div className="relative aspect-[10/8] w-full overflow-hidden">
      <img
        src={image}
        alt={name}
        ...
        {socials && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex gap-4">
              {socials.website && (
                <a href={socials.website} target="_blank" rel="noopener noreferrer" className="rounded-full bg-white/20 p-3 backdrop-blur-sm transition-all hover:bg-green-500/30 hover:scale-110" title={socials.websiteLabel || 'Website'}>
              ...
              {socials.linkedin && (
                <a href={`https://linkedin.com/in/${socials.linkedin}`} target="_blank" rel="noopener noreferrer" className="rounded-full bg-white/20 p-3 backdrop-blur-sm transition-all hover:bg-blue-500/30 hover:scale-110" title="LinkedIn">
              ...
              {socials.github && (
                <a href={`https://github.com/${socials.github}`} target="_blank" rel="noopener noreferrer" className="rounded-full bg-white/20 p-3 backdrop-blur-sm transition-all hover:bg-gray-600/30 hover:scale-110" title="GitHub">
              ...
              {socials.instagram && (
                <a href={`https://instagram.com/${socials.instagram}`} target="_blank" rel="noopener noreferrer" className="rounded-full bg-white/20 p-3 backdrop-blur-sm transition-all hover:bg-pink-500/30 hover:scale-110" title="Instagram">
              ...
              {socials.twitter && (
                <a href={`https://twitter.com/${socials.twitter}`} target="_blank" rel="noopener noreferrer" className="rounded-full bg-white/20 p-3 backdrop-blur-sm transition-all hover:bg-blue-400/30 hover:scale-110" title="Twitter">
  ...
  return (
    <section id="team" className="bg-black py-20 text-white">
      <div className="container mx-auto px-4 md:px-6">
        ...
        {/* Founders */}
        <div className="mb-20">
          <h3 className="mb-10 text-center text-3xl font-bold md:text-4xl">Founding Team</h3>
        ...
        {/* Technical Leads */}
        <div className="mb-20">
          <h3 className="mb-10 text-center text-3xl font-bold md:text-4xl">Technical Team</h3>
          <div className="mx-auto flex max-w-3xl flex-wrap justify-center gap-8">
            {technicalLeads.map((lead, index) => (
              <div key={`tech-lead-${index}`} className="w-full sm:w-64">
                <TeamMember {...lead} />
              </div>
            ))}
          </div>
        ...
        {/* Corporate Leads */}
        <div className="mb-20">
          <h3 className="mb-10 text-center text-3xl font-bold md:text-4xl">Corporate Team</h3>
          <div className="mx-auto flex max-w-3xl flex-wrap justify-center gap-8">
            {corporateLeads.map((lead, index) => (
              <div key={`corp-lead-${index}`} className="w-full sm:w-64">
                <TeamMember {...lead} />
              </div>
            ))}
          </div>
        ...
        {/* Creative Leads */}
        <div>
          <h3 className="mb-10 text-center text-3xl font-bold md:text-4xl">Creative Team</h3>
          <div className="mx-auto flex max-w-3xl flex-wrap justify-center gap-8">
            {creativeLeads.map((lead, index) => (
              <div key={`creative-lead-${index}`} className="w-full sm:w-64">
                <TeamMember {...lead} />
              </div>
            ))}
          </div>
